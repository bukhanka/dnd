...
def generate_image_for_scene(scene_description: str) -> str:
    prompt = f"Fantasy scene: {scene_description}"
...
def start_new_adventure(party_members: Dict[str, str], difficulty: str) -> Tuple[Dict, str]:
    ...
    initial_image_url = generate_image_for_scene(dm_intro)
...
def display_generated_image(image_url: str):
...
def main():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["Game", "Party", "Inventory", "Image Generation"])
    ...
    with tab4:
        ...
        image_prompt = st.text_area("Enter a prompt for image generation:", 
                                    "A magical fantasy landscape with a dragon")
        if st.button("Generate Image"):
            with st.spinner("Conjuring an image from the realms of imagination..."):
...
def display_adventure_log():
    if st.session_state.game_state is not None:
        ...
        if "current_image_url" in st.session_state.game_state:
            st.image(st.session_state.game_state["current_image_url"], caption="Current Scene", use_column_width=True)
        ...
        if st.button("🎲 Play Next Turn"):
            ...
            with st.spinner("The dice are rolling..."):
                ...
                # Generate a new image for the current scene
                new_image_url = generate_image_for_scene(dm_response)
                st.session_state.game_state["current_image_url"] = new_image_url
